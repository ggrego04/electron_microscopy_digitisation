...
for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
    path = TRAIN_PATH + id_
    img = imread(path + '/images/sample.png')[:, :, :IMG_CHANNELS]
...
# save the model
model.save('model_for_segmentation.h5')
# load the model
model = tf.keras.models.load_model("model_for_segmentation.h5")
# read the test image
image = imread('test_image.png')[:, :, :IMG_CHANNELS]
sizes_test.append([image.shape[0], image.shape[1]])
image = resize(image, (IMG_HEIGHT, IMG_WIDTH), mode='constant', preserve_range=True)
X_test[-1] = image
idx = random.randint(0, len(X_train))
# predict a train image and the test image
...
# Perform a sanity check on a training sample
ix = 0
imshow(X_train[ix])
plt.show()
...
# Perform a sanity check on the test sample
ix = -1
imshow(X_test[ix])
plt.show()
