...
# this function removes the background around the membrane
def remove_background(img, new_img):
    img_y = img.shape[0]
    img_x = img.shape[1]
    # if the mask haw black pixels remove them from the original
...
def find_skeleton(skeleton):
    ...
    # present the plot with the skeleton
    plt.imshow(new_img, cmap='gray')
    plt.axis('off')
    plt.contour(fil.skeleton_longpath, colors='b')
    # save the plot as image
...
# this function draws the contour area using the mask from Unet
def get_contour(image, mask):
    img_y = image.shape[0]
    img_x = image.shape[1]
    ...
    for x in range(0, img_x):
        flag = 0
        for y in range(0, img_y):
            # make the first white pixel red
            if (mask[y][x] == 255 and flag == 0):
                image[y][x][0] = 0
                image[y][x][1] = 0
                image[y][x][2] = 255
                flag = 1
    flag = 0
    for x in range(img_x - 1, 0, -1):
        flag = 0
        for y in range(img_y - 1, 0, -1):
            # make the first white pixel red
            if (mask[y][x] == 255 and flag == 0):
                image[y][x][0] = 0
                image[y][x][1] = 0
...
if __name__ == "__main__":
    ...
    new_img = deepcopy(img)
    t_img = deepcopy(img)
    # resize the image to 128x128 to be accepted by the Unet
    temp_img = resize(temp_img, (128, 128), mode='constant', preserve_range=True)
    # call the unet method and get the returned mask
    m = prediction(temp_img)
    # resize the mask to original dimensions
    m = resize(m, dim, mode='constant', preserve_range=True)

    th_ls, threshed_ls = cv2.threshold(m, 240, 255, cv2.THRESH_BINARY_INV)

    # Morph-op to remove noise
    kernel_ls = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (15, 15))
    # even out the edges
    m = cv2.morphologyEx(threshed_ls, cv2.MORPH_CLOSE, kernel_ls)
    m = cv2.morphologyEx(m, cv2.MORPH_OPEN, kernel_ls)
    m = 255 - m
    # remove the background of the membrane
    remove_background(new_img, m)
    # draw the contour around the mask
    get_contour(img, m)
